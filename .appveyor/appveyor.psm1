...
function Start-AppveyorTestScriptTask
{
    ...
    $result = Invoke-AzureStorageDscTestSuite -TestResultsFile $testResultsFile `
...
function Start-AppveyorAfterTestTask
{
    ...
    $mainModulePath = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER -ChildPath "modules\AzureStorageDsc"

    # Write the PowerShell help files
    $docoPath = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER `
                          -ChildPath "modules\AzureStorageDsc\en-US"
    ...
    $zipFileName = "AzureStorageDsc_$($env:APPVEYOR_BUILD_VERSION)_wikicontent.zip"
    ...
    $manifest = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER -ChildPath "modules\AzureStorageDsc\AzureStorageDsc.psd1"
    (Get-Content $manifest -Raw).Replace("1.0.0.0", $env:APPVEYOR_BUILD_VERSION) | Out-File $manifest
    $zipFileName = "AzureStorageDsc_$($env:APPVEYOR_BUILD_VERSION).zip"
    ...
    $nuspecParams = @{
        ...
        packageDescription = "AzureStorageDsc"
    ...
    Start-Process -FilePath "nuget" -Wait -ArgumentList @(
        "pack",
        ".\AzureStorageDsc.nuspec",
        "-outputdirectory $env:APPVEYOR_BUILD_FOLDER"
    )
    $nuGetPackageName = "AzureStorageDsc." + $env:APPVEYOR_BUILD_VERSION + ".nupkg"
